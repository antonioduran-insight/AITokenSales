...
**`icp_score` and the HOT/WARM/COLD tier are not trustworthy — measured 05/08/2026, fix deliberately deferred.** The scorer lives in the scraper repo (`scraper/icp_scorer.py`, which carries the full write-up at the top of the file). Summary of what the number actually is: three of its five components return a hardcoded constant (40 of 100 points); `AI_SIGNAL_KEYWORDS` contains the bare string `"ai"` matched with `in`, so it fires on av**ai**lable, em**ai**l, det**ai**l, ret**ail**, T**ai**pei — effectively a fourth constant worth 20 more; and `PRIORITY_TITLES` is also substring-matched, so **every "Director" scores as a CTO** (dire**cto**r) and **every "Coordinator" as a COO**, while `"Chief Technology Officer"`, `"VP of Engineering"` and `"Head of Product"` score **zero**. Net: 85 of 100 points measure nothing and the other 30 are wrong in both directions, so nearly every lead with a bio lands on 60 (WARM) and rises to 90 (HOT) only on a substring accident. Separately, the lists encode a single ICP (technical buyers who mention AI) for **all** orgs, even though `org_combos` already lets each org pick its own target titles — the scraper searches the customer's titles and then scores against a different, hardcoded profile. Do not build features that rely on the score meaning something until this is fixed; fixing it re-scores every existing lead and will visibly change temperatures SDRs already know.
