...
`/history`, `/audit`, `/admin/users`, `/bridge`, `(scraper)/dashboard`, `(scraper)/run`, and `(scraper)/export` used to rely entirely on RLS to keep an SDR from seeing their contents: the page still mounted and rendered (an empty shell, since RLS blocked every query), instead of being blocked at the route level like `/global-admin` already was. No data actually leaked — RLS held — but an empty-but-mounted page is a fragile second layer: any future component doing a service-role fetch or showing an aggregate count on one of these pages could leak data without anyone noticing. Fixed with `blockSdrAccess()` (`src/lib/utils/route-guard.ts`), an SSR check mirroring `GlobalAdminLayout`'s own pattern, called at the top of each page before anything renders. Every one of these pages used to be a single client component, so each needed the same small split: `page.tsx` is now a thin async server component that does only the check, delegating to a same-named `*Client.tsx` (`HistoryClient.tsx`, `DashboardClient.tsx`, `RunClient.tsx`, `ExportClient.tsx`, `BridgeClient.tsx`) holding the original content unchanged.

**Also found while fixing this**: `/admin/users` had *no* route-level gate at all before this fix, despite FUNC-F12 citing it as an already-correctly-blocked reference case — it was exposed to the exact same gap as `/history`/`/audit`, just not yet caught by testing. Fixed alongside the rest.

### UX/performance batch (F1/F2/F6/F10/F13/F14/F17, FUNC-F1/F2/F4/F6/F7/F13, S1/S2)

- **New Run**: the "Run" button already showed disabled+"Starting…" immediately on click — this batch adds a spinner icon on top of that, since testers read the subtle text-only state as "no feedback" under slow/concurrent load.
- **Progress bar simplified**: the backend only ever reports `pending → running → completed/failed`, never the intermediate `scraping/scoring/drafting` states the old 4-dot bar pretended to track — confirmed no code anywhere writes those values. Replaced with an honest single "Running…" state; the fabricated per-stage dots are gone. (Real per-stage progress would need the scraper backend instrumented to emit those states — a separate, larger backend project, not done here.)
- **Sidebar prefetching**: every nav `<Link>` now sets `prefetch={false}` — the sidebar's ~10 always-visible links were each prefetching their route's RSC payload (including, since FUNC-F12, an SSR auth round-trip) on every mount. Confirmed both New Run's and Bridge's status polling already stop correctly once a run reaches a terminal state — no fix needed there.
- **`PATCH /api/prospects` latency (F17, measured ~4.9s)**: the admin-role check and the target-SDR validation were two independent reads running sequentially; they now run via `Promise.all`. Also added `prospects_assigned_to_created_at` (see `supabase/migrations/20260727_prospects_assigned_to_index.sql`, **run by hand**) — the bulk-transfer-by-quantity query filtered on `assigned_to` alone, which only existed as the trailing column of an unrelated composite unique index.
- **"Reassign selected"**: the bulk-action bar's checkbox selection could only Delete. Added a second action that reassigns exactly the checked leads (not an arbitrary "oldest N") to one SDR, logging each lead by name in the audit log — the existing by-quantity reassign only ever logged a count, which is why the audit log showed "—" for the lead column.
- **Kanban cards were already clickable** and already opened the same drawer Leads uses (full info, messages, notes) — FUNC-F7/S1 described a gap that didn't exist in the current code. Combined with the inline-edit below, an SDR can already see and edit everything from a Kanban card without visiting Leads.
- **Inline-editable messages (FUNC-F6)**: `custom1`/`custom2` in the lead drawer's Messages tab were read-only. Now editable in place (textarea + Save/Cancel), same drawer used by both Kanban and Leads.
- **Custom 3 removed (FUNC-F4)**: `scraper_leads` only ever had two message slots (`custom1`/`custom2`) — `custom3` was a leftover from when the product generated three, never wired to auto-generation. Removed from the drawer, the manual-create form, and CSV import mapping; the underlying `prospects.custom3` column is untouched for any org still relying on it as a plain manual field.
- **Combo codes → labels (FUNC-F1)**: History, Leads, and the Kanban card badge all rendered the raw internal code (`combo_D`) instead of the combo's human name ("CTO / VP Engineering"). New `useComboLabels()` hook (`src/lib/hooks/useComboLabels.ts`) resolves codes via the same `/api/scraper-combos` catalogue New Run's picker already uses.
- **Audit log status labels (FUNC-F13)**: "Status Changed" entries showed the raw enum (`new → connection_sent`) instead of the translated label used everywhere else.
- **Scraped-name cleanup (FUNC-F2)**: names like "Jassen Castillo - Software Engineer" (a title the scraper occasionally leaves stuck onto the name field) are now split at the first ` - `/` – `/` — `/` | ` separator and trimmed to just the name, applied where scraped leads get written to `prospects` (`assignRunLeads()`). Manually-created/CSV-imported names are left untouched — only scraper output has this artifact.
- **Per-SDR Kanban view (FUNC-S2)**: admins can now filter the consolidated board down to one SDR's leads via a dropdown next to the area filter, purely client-side (no extra query — the SDR's name is already joined onto each prospect row).
- **Transient move highlight (F13)**: a card that just changed columns gets a brief accent-colored glow (fades out over ~1.6s) instead of just disappearing and reappearing with no visual continuity.
- **"Global" market filter (F14) — discarded, doesn't apply**: "Global" was never an `area_id`/region value; it only ever existed as a free-text fallback on `runs.market` for a degenerate case the region-first New Run picker can't really produce anymore. There's no missing filter option to add — the ticket's premise didn't hold up once traced through the code.
