      ...
      <div style={{ backgroundColor: 'var(--crm-surface)', border: '1px solid var(--crm-border)', borderRadius: 12, padding: '20px 24px', marginBottom: 20 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderTop: '1px solid var(--crm-border)', paddingTop: 16 }}>
          ...
          <button onClick={handleDownload} disabled={!selectedRunId || leads.length === 0 || downloading}
            style={{ display: 'flex', alignItems: 'center', gap: 8, backgroundColor: !selectedRunId || leads.length === 0 || downloading ? 'var(--crm-border)' : 'var(--crm-accent)', color: '#FFF', padding: '9px 18px', borderRadius: 8, border: 'none', cursor: !selectedRunId || leads.length === 0 || downloading ? 'default' : 'pointer', fontSize: 13, fontWeight: 600 }}>
