  ...
  return (
    <div style={S.page}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 12, marginBottom: 24 }}>
        {[
          { label: t('totalRuns'), value: runs.length, icon: <Activity size={16} color="var(--crm-accent)" /> },
          { label: t('totalLeadsRequested'), value: runs.reduce((s, r) => s + (r.total_leads_requested || 0), 0), icon: <Activity size={16} color="#22C55E" /> },
        ].map(s => (
      ...
      {activeRun && (
        <div style={{ borderRadius: 12, border: '1px solid #6C63FF40', backgroundColor: '#6C63FF10', padding: '12px 16px', display: 'flex', alignItems: 'center', flexWrap: 'wrap', rowGap: 8, gap: 12, marginBottom: 24 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <p style={{ fontSize: 13, color: 'var(--crm-text-primary)', fontWeight: 600, margin: 0 }}>{t('activeRun', { market: runMarket(activeRun) })}</p>
          ...
          <Link href="/history" style={{ fontSize: 11, color: 'var(--crm-accent)', whiteSpace: 'nowrap' }}>{t('viewHistory')} →</Link>
        </div>
      )}
          ...
          <div style={S.card}>
            ...
            {(latestRun.total_leads_requested ?? 0) > 0 && (
              <div style={{ marginBottom: 12 }}>
                <div style={{ fontSize: 12, color: 'var(--crm-text-muted)' }}>{t('leadsRequested', { count: latestRun.total_leads_requested ?? 0 })}</div>
              </div>
            )}
            <div style={{ display: 'flex', gap: 8 }}>
              <Link href="/history" style={{ fontSize: 12, backgroundColor: 'var(--crm-accent)', color: '#FFF', padding: '6px 14px', borderRadius: 7, textDecoration: 'none', fontWeight: 600 }}>{t('viewHistory')}</Link>
            </div>
          </div>
        </section>
      )}
            ...
            {runs.slice(1, 6).map((run, i) => (
              <div key={run.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 16px', borderTop: i > 0 ? '1px solid var(--crm-border)' : undefined }}>
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <p style={{ fontSize: 11, color: 'var(--crm-text-muted)', margin: '2px 0 0' }}>{new Date(run.created_at).toLocaleDateString()} · {t('leadsReq', { count: run.total_leads_requested ?? 0 })}</p>
                </div>
      ...
      {loading && <p style={{ color: 'var(--crm-text-muted)', fontSize: 13, textAlign: 'center', padding: 40 }}>{tc('loading')}</p>}
      {!loading && runs.length === 0 && (
        <div style={{ textAlign: 'center', padding: '60px 0' }}>
          <p style={{ color: 'var(--crm-text-muted)', marginBottom: 16 }}>{t('noRunsYet')}</p>
