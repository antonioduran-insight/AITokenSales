...
export async function POST(req: NextRequest) {
  try {
    ...
    if (SCRAPER_API) {
      ...
      if (!scraperRes.ok) {
        const errText = await scraperRes.text().catch(() => 'Scraper unreachable')
        await admin.from('runs').update({ status: 'failed', error_message: errText }).eq('id', run.id)
        return NextResponse.json({ error: `Scraper backend error: ${errText}` }, { status: 502 })
      }
    }
