    ...
    <div style={{ padding: '4px 0' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
        <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--crm-text-secondary)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
          Chats de venta {convs.length > 0 && <span style={{ fontSize: 11, backgroundColor: '#6C63FF20', color: 'var(--crm-accent)', borderRadius: 10, padding: '1px 7px', marginLeft: 6 }}>{convs.length}</span>}
        </span>
        <button
      ...
      {loading && <p style={{ color: 'var(--crm-text-muted)', fontSize: 13 }}>Cargando...</p>}
      {!loading && convs.length === 0 && <p style={{ color: 'var(--crm-text-muted)', fontSize: 13 }}>Sin chats subidos.</p>}
        ...
        <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.8)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 200 }}>
          <div style={{ backgroundColor: 'var(--crm-surface)', border: '1px solid var(--crm-border)', borderRadius: 12, padding: 24, width: 560, maxWidth: '95vw', maxHeight: '90vh', overflowY: 'auto' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
              <h3 style={{ margin: 0, fontSize: 16, fontWeight: 700, color: 'var(--crm-text-primary)' }}>Chat de venta — {prospectName}</h3>
              <button onClick={() => { setAddOpen(false); setChatContent(''); setError('') }} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--crm-text-muted)' }}><X size={16} /></button>
            </div>
            <label style={{ fontSize: 12, color: 'var(--crm-text-muted)', display: 'block', marginBottom: 6 }}>Pegá la conversación completa con el cliente:</label>
            <textarea
              value={chatContent}
